...
21. Policy freeze/rollback controls are valid only for governed digests in the same policy lineage.
22. A frozen or inactive policy cannot mint new authority, including through a cached signed policy artifact; the control state is checked on every authority issuance.
23. Policy control actions are immutable and replay-protected by persisted unique action ID and nonce.
